...
5. La matriz local autenticada y la ejecución remota de OWASP ZAP API baseline pasaron; la matriz autenticada local ejercitó Tenant Owner, Company Owner, Sales, Warehouse, Logistics y Buyer con `FAIL-NEW: 0` en los seis perfiles (`WARN-NEW: 6–7`, `PASS: 110–111`). El baseline remoto reportó `FAIL-NEW: 0`, `WARN-NEW: 7`, `PASS: 60` sobre 13 URLs. Trivy/SBOM remoto no detectó vulnerabilidades, secretos ni misconfiguraciones. Esto es evidencia ASVS-L2 objetivo, no certificación ASVS.
