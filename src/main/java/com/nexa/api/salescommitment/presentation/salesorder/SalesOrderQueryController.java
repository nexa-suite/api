...
public final class SalesOrderQueryController {
	...
	public SalesOrderPageResponse list(@RequestAttribute(ACCESS_CONTEXT_ATTRIBUTE) CurrentAccessContext context,
			@RequestParam(required = false) @Pattern(regexp = "(?i)PENDING|CONFIRMED|IN_FULFILLMENT|PARTIALLY_FULFILLED|FULFILLED|PARTIALLY_DELIVERED|COMPLETED|REJECTED|CANCELLED") String status,
	...
	public com.nexa.api.salescommitment.presentation.salesorder.response.FulfillmentCandidatePageResponse candidates(@RequestAttribute(ACCESS_CONTEXT_ATTRIBUTE) CurrentAccessContext context,
		...
		return new com.nexa.api.salescommitment.presentation.salesorder.response.FulfillmentCandidatePageResponse(value.items().stream().map(mapper::candidate).toList(), value.page(), value.size(), value.total());
	}
}
