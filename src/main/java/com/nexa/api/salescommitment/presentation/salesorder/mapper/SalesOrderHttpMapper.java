...
public @Component class SalesOrderHttpMapper {
	...
	public FulfillmentCandidateResponse candidate(FulfillmentCandidateView value) { return new FulfillmentCandidateResponse(value.id(), value.number(), value.clientAccountId(), value.status(), value.version(), value.lines().stream().map(line -> new FulfillmentCandidateResponse.Line(line.catalogItemId(), line.itemName(), line.quantity(), line.unit())).toList()); }
