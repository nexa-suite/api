...
@Component
@Profile("!test")
public final class OrderSummaryCsvRenderer implements BusinessDocumentRenderer {
    @Override public boolean supports(DocumentProjection projection, BusinessDocumentFormat format) { return projection.type() == BusinessDocumentType.ORDER_SUMMARY && format == BusinessDocumentFormat.CSV; }
    @Override public RenderedDocument render(DocumentProjection projection, BusinessDocumentFormat format) {
        RendererSupport.require(projection, com.nexa.api.businessdocuments.application.model.BusinessDocumentProjections.OrderSummaryProjection.class, BusinessDocumentType.ORDER_SUMMARY, BusinessDocumentFormat.CSV);
