...
public class JdbcOrganizationAdministrationAdapter implements OrganizationAdministrationPort {
	...
	private String membershipSql() { return "select m.id,m.workspace_id,m.user_id,u.email,u.display_name,m.membership_type,m.status,m.version,case when m.membership_type='BUYER' then array['BUYER'] else coalesce(array_agg(r.role) filter (where r.role is not null), array[]::varchar[]) end as roles,coalesce((select array_agg(a.role_id::text order by a.role_id) from tenant_management.membership_role_definition a where a.membership_id=m.id), array[]::text[]) as role_definition_ids,coalesce((select array_agg(distinct p.permission_key order by p.permission_key) from tenant_management.membership_role_definition a join tenant_management.role_permission p on p.role_id=a.role_id where a.membership_id=m.id), array[]::text[]) as dynamic_permissions from tenant_management.workspace_membership m join tenant_management.workspace w on w.id=m.workspace_id join iam.user_account u on u.id=m.user_id left join tenant_management.membership_role_assignment r on r.membership_id=m.id"; }
	...
	private WorkspaceMembershipSummary membership(java.sql.ResultSet rs) throws java.sql.SQLException {
		...
		Set<MembershipRole> fixedRoles = roles.stream().map(MembershipRole::from).collect(java.util.stream.Collectors.toUnmodifiableSet());
		Set<String> roleIds = new LinkedHashSet<>(fixedRoles.stream().map(role -> RoleDefinitionId.system(role.name()).toString()).collect(java.util.stream.Collectors.toUnmodifiableSet()));
		roleIds.addAll(stringArray(rs.getArray(10)));
		Set<String> permissions = new LinkedHashSet<>(fixedRoles.isEmpty() ? Set.of() : PermissionCatalog.compatibilityCodes(PermissionCatalog.forBuiltInRoles(fixedRoles)));
		permissions.addAll(stringArray(rs.getArray(11)));
