...
public class JdbcOrganizationAdministrationAdapter implements OrganizationAdministrationPort {
	...
	@Override @Transactional public int updateStatus(String tenantId,String membershipId,String status,long version){
		lockMembershipWorkspace(tenantId, membershipId);
		return jdbc.update("update tenant_management.workspace_membership m set status=?,updated_at=current_timestamp,version=m.version+1 from tenant_management.workspace w where m.workspace_id=w.id and w.tenant_id=? and m.id=? and m.version=? and (? or not exists (select 1 from tenant_management.membership_role_assignment current_role where current_role.membership_id=m.id and current_role.role='TENANT_ADMIN') or exists (select 1 from tenant_management.workspace_membership other join tenant_management.membership_role_assignment other_role on other_role.membership_id=other.id where other.workspace_id=m.workspace_id and other.id<>m.id and other.status='ACTIVE' and other_role.role='TENANT_ADMIN'))",status,uuid(tenantId),uuid(membershipId),version,!"DISABLED".equals(status));
	}
