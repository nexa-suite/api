...
public class JdbcOrganizationAdministrationAdapter implements OrganizationAdministrationPort {
	...
	@Override public void createWorkspaceMembership(String tenantId, String workspaceId, java.util.UUID userId, java.util.Set<String> roles, java.time.Instant createdAt) {
		...
		for (String role : roles) jdbc.update("insert into tenant_management.membership_role_assignment (membership_id,tenant_id,workspace_id,role,assigned_at) values (?,?,?,?,?)", membershipId, uuid(tenantId), uuid(workspaceId), role, java.sql.Timestamp.from(createdAt));
	}
	...
	@Override @Transactional public int updateRoles(String tenantId,String membershipId,Set<String> roles,long version){
		...
		int updated = jdbc.update("update tenant_management.workspace_membership m set updated_at=current_timestamp,version=m.version+1 from tenant_management.workspace w where m.workspace_id=w.id and w.tenant_id=? and m.id=? and m.membership_type='INTERNAL' and m.version=? and (cast(? as boolean) or not exists (select 1 from tenant_management.membership_role_assignment current_assignment where current_assignment.membership_id=m.id and current_assignment.role='TENANT_ADMIN') or exists (select 1 from tenant_management.workspace_membership other join tenant_management.membership_role_assignment other_assignment on other_assignment.membership_id=other.id where other.workspace_id=m.workspace_id and other.id<>m.id and other.status='ACTIVE' and other_assignment.role='TENANT_ADMIN'))",uuid(tenantId),uuid(membershipId),version,roles.contains("TENANT_ADMIN"));
		if (updated == 0) return 0;
		jdbc.update("delete from tenant_management.membership_role_assignment where membership_id=?",uuid(membershipId));
		for (String role : roles) jdbc.update("insert into tenant_management.membership_role_assignment (membership_id,tenant_id,workspace_id,role,assigned_at) select m.id,w.tenant_id,m.workspace_id,?,current_timestamp from tenant_management.workspace_membership m join tenant_management.workspace w on w.id=m.workspace_id where m.id=? and w.tenant_id=?",role,uuid(membershipId),uuid(tenantId));
		return updated;
	}
	@Override @Transactional public int updateStatus(String tenantId,String membershipId,String status,long version){
		lockMembershipWorkspace(tenantId, membershipId);
		return jdbc.update("update tenant_management.workspace_membership m set status=?,updated_at=current_timestamp,version=m.version+1 from tenant_management.workspace w where m.workspace_id=w.id and w.tenant_id=? and m.id=? and m.version=? and (cast(? as boolean) or not exists (select 1 from tenant_management.membership_role_assignment current_assignment where current_assignment.membership_id=m.id and current_assignment.role='TENANT_ADMIN') or exists (select 1 from tenant_management.workspace_membership other join tenant_management.membership_role_assignment other_assignment on other_assignment.membership_id=other.id where other.workspace_id=m.workspace_id and other.id<>m.id and other.status='ACTIVE' and other_assignment.role='TENANT_ADMIN'))",status,uuid(tenantId),uuid(membershipId),version,!"DISABLED".equals(status));
	}
	...
	private String membershipSql() { return "select m.id,m.workspace_id,m.user_id,u.email,u.display_name,m.membership_type,m.status,m.version,case when m.membership_type='BUYER' then array['BUYER'] else coalesce(array_agg(r.role) filter (where r.role is not null), array[]::varchar[]) end as roles from tenant_management.workspace_membership m join tenant_management.workspace w on w.id=m.workspace_id join iam.user_account u on u.id=m.user_id left join tenant_management.membership_role_assignment r on r.membership_id=m.id"; }
