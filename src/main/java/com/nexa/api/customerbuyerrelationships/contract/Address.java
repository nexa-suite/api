...
public record Address(String addressType, String line, String reference, String countryCode,
    ...
    public Address {
        ...
        if (!"PE".equals(countryCode)) {
            throw new CustomerRelationshipInvariantViolation("Only Peru delivery addresses are supported");
        }
        ...
        if ((latitude == null) != (longitude == null)) throw new CustomerRelationshipInvariantViolation("Coordinates must be supplied together");
        if (latitude != null && (latitude.compareTo(BigDecimal.valueOf(-90)) < 0 || latitude.compareTo(BigDecimal.valueOf(90)) > 0
            ...
            throw new CustomerRelationshipInvariantViolation("Coordinates are invalid");
        }
        ...
        if (!Set.of("MANUAL", "SAVED", "CURRENT_LOCATION", "MAP_PIN", "PLACES").contains(source)) {
            throw new CustomerRelationshipInvariantViolation("Address source is invalid");
        }
    }
    ...
    private static String required(String value, String label, int max) {
        if (value == null || value.isBlank() || value.trim().length() > max) {
            throw new CustomerRelationshipInvariantViolation(label + " is invalid");
        }
        return value.trim();
    }
    ...
    private static String optional(String value, String fallback, int max) {
        ...
        if (value.trim().length() > max) throw new CustomerRelationshipInvariantViolation("Address field is too long");
        return value.trim();
    }
}
