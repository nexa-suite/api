package com.nexa.api.customerbuyerrelationships.presentation.clientaccountaddress;
...
import com.nexa.api.customerbuyerrelationships.presentation.contract.DeliveryAddressRequest;
...
public final class ClientAccountAddressController {
    ...
    public ResponseEntity<AddressResponse> create(@RequestAttribute(ACCESS_CONTEXT) CurrentAccessContext context,
        ...
        return ResponseEntity.status(201).eTag(CustomerRelationshipHttpHeaders.etag(value.version())).body(AddressResponse.from(value));
    }
    ...
    public ResponseEntity<AddressResponse> update(@RequestAttribute(ACCESS_CONTEXT) CurrentAccessContext context,
        ...
        var value = addresses.update(context, clientAccountId, addressId,
                ...
                CustomerRelationshipHttpHeaders.requireVersion(ifMatch));
        return ResponseEntity.ok().eTag(CustomerRelationshipHttpHeaders.etag(value.version())).body(AddressResponse.from(value));
    }
    ...
    public ResponseEntity<AddressResponse> setDefault(@RequestAttribute(ACCESS_CONTEXT) CurrentAccessContext context,
                                                      @PathVariable String clientAccountId, @PathVariable String addressId,
                                                      @RequestHeader(name = "If-Match", required = false) String ifMatch) {
        var value = addresses.setDefault(context, clientAccountId, addressId, CustomerRelationshipHttpHeaders.requireVersion(ifMatch));
        return ResponseEntity.ok().eTag(CustomerRelationshipHttpHeaders.etag(value.version())).body(AddressResponse.from(value));
    }
    ...
    public ResponseEntity<AddressResponse> deactivate(@RequestAttribute(ACCESS_CONTEXT) CurrentAccessContext context,
                                                      @PathVariable String clientAccountId, @PathVariable String addressId,
                                                      @RequestHeader(name = "If-Match", required = false) String ifMatch) {
        var value = addresses.deactivate(context, clientAccountId, addressId, CustomerRelationshipHttpHeaders.requireVersion(ifMatch));
        return ResponseEntity.ok().eTag(CustomerRelationshipHttpHeaders.etag(value.version())).body(AddressResponse.from(value));
    }
