...
public final class WarehouseController {
    ...
    @GetMapping("/inventory/lots/{lotId}") public WarehouseOperationsService.LotSummary lot(@RequestAttribute(ACCESS) CurrentAccessContext c,@PathVariable String lotId){return service.lots(c,null,null,null,null,0,100,"receivedAt").items().stream().filter(l->l.id().equals(lotId)).findFirst().orElseThrow(()->new WarehouseOperationsService.WarehouseException("INVENTORY_LOT_NOT_FOUND",true));}
