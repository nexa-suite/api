...
public class ApiSecurityConfiguration {
	...
	CorsConfigurationSource corsConfigurationSource(Environment environment) {
		...
		configuration.setAllowedHeaders(List.of("Authorization", "Content-Type", "If-Match", "Idempotency-Key", "X-Resume-Token", "X-Organization-Registration-Token", "X-Correlation-Id", "X-Trace-ID", "X-Nexa-Surface", "X-Nexa-Client"));
	...
	ProblemDetailAccessDeniedHandler accessDeniedHandler(ObjectMapper objectMapper,
			org.springframework.beans.factory.ObjectProvider<com.nexa.api.tenantaccessgovernance.iam.application.port.out.SecurityAuditPort> audit,
			org.springframework.beans.factory.ObjectProvider<com.nexa.api.shared.application.port.out.SecurityMetricsPort> metrics) {
	...
	CurrentAccessContextFilter currentAccessContextFilter(
			com.nexa.api.tenantaccessgovernance.tenantmanagement.application.port.in.ResolveCurrentAccessContextUseCase accessContext,
			com.nexa.api.tenantaccessgovernance.iam.application.port.in.ValidateAccessSessionUseCase accessSession,
