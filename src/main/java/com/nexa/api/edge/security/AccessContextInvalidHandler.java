...
final class AccessContextInvalidHandler implements AccessDeniedHandler {
	...
	private final ObjectProvider<SecurityMetricsPort> metrics;

	AccessContextInvalidHandler(ObjectMapper objectMapper, ObjectProvider<SecurityAuditPort> audit,
			ObjectProvider<SecurityMetricsPort> metrics) {
	...
	public void handle(HttpServletRequest request, HttpServletResponse response, AccessDeniedException exception)
			throws IOException {
		SecurityMetricsPort metric = metrics.getIfAvailable();
		...
		if (port != null) {
			...
			String correlation = request.getAttribute(com.nexa.api.edge.http.CorrelationIdFilter.ATTRIBUTE_NAME) == null
					? "unknown" : request.getAttribute(com.nexa.api.edge.http.CorrelationIdFilter.ATTRIBUTE_NAME).toString();
