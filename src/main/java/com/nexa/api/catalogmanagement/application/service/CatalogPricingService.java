...
public final class CatalogPricingService implements CatalogPricingUseCase {
    ...
    @Override public CatalogManagementModels.PriceView create(CatalogScope scope, UUID productId, BigDecimal amount, String currency, Instant validFrom, Instant validUntil, String sourceCode, String sourceDescription) { authorization.require(CatalogPermissions.PRICE_MANAGE); return port.create(scope, productId, amount, currency, validFrom, validUntil, sourceCode, sourceDescription); }
    @Override public CatalogManagementModels.PriceView create(CatalogScope scope, UUID productId, BigDecimal amount, String currency, Instant validFrom, Instant validUntil, String sourceCode, String sourceDescription, String idempotencyKey) { authorization.require(CatalogPermissions.PRICE_MANAGE); return port.create(scope, productId, amount, currency, validFrom, validUntil, sourceCode, sourceDescription, idempotencyKey); }
    @Override public CatalogManagementModels.PriceView cancel(CatalogScope scope, UUID priceId, long version) { authorization.require(CatalogPermissions.PRICE_MANAGE); return port.cancel(scope, priceId, version); }
}
