...
public class CatalogSkuPersistenceBootstrap {
    ...
    private void reconcileProduct(ResultSet rs, Instant now) throws SQLException {
        ...
        UUID familyId = jdbc.query("select id from catalog_management.product_family where tenant_id=? and workspace_id=? and family_code=?",
                (r, n) -> r.getObject(1, UUID.class), tenantId, workspaceId, mapping.familyCode()).stream().findFirst().orElse(null);
        if (familyId == null) {
            UUID candidate = UUID.nameUUIDFromBytes((tenantId + ":" + workspaceId + ":family:" + mapping.familyCode()).getBytes(StandardCharsets.UTF_8));
            jdbc.update("insert into catalog_management.product_family (id,tenant_id,workspace_id,family_code,name,description,category_id,brand_id,storage_family,status,created_at,updated_at) values (?,?,?,?,?,?,?,?,?,?,?,?) on conflict (tenant_id,workspace_id,family_code) do nothing",
                    candidate, tenantId, workspaceId, mapping.familyCode(), mapping.familyName(), rsSafe(rs, "description", ""), rs.getObject("category_id", UUID.class), rs.getObject("brand_id", UUID.class), rs.getString("storage_temperature"), rs.getString("status"), rs.getTimestamp("created_at"), rs.getTimestamp("updated_at"));
            familyId = jdbc.queryForObject("select id from catalog_management.product_family where tenant_id=? and workspace_id=? and family_code=?", UUID.class, tenantId, workspaceId, mapping.familyCode());
        } else {
            jdbc.update("update catalog_management.product_family set name=?,updated_at=? where tenant_id=? and workspace_id=? and id=?", mapping.familyName(), java.sql.Timestamp.from(now), tenantId, workspaceId, familyId);
        }
