...
public @Component class SalesOrderHttpMapper {
	public SalesOrderResponse response(SalesOrderView value) { return new SalesOrderResponse(value.id(), value.number(), value.tenantId(), value.workspaceId(), value.clientAccountId(), value.buyerMembershipId(), value.sourcePurchaseRequestId(), value.currency(), value.total(), value.status(), value.createdAt(), value.confirmedAt(), value.rejectionReason(), value.version(), value.lines().stream().map(this::line).toList()); }
	...
	private SalesOrderLineResponse line(SalesOrderLineView line) { return new SalesOrderLineResponse(line.catalogItemId(), line.itemName(), line.quantity(), line.unit(), line.unitPriceAmount(), line.unitPriceCurrency()); }
}
