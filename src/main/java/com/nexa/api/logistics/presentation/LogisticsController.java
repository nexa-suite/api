...
public final class LogisticsController {
    ...
    @GetMapping("/dispatch-orders/{id}") public DispatchResponse dispatch(@RequestAttribute(ACCESS) CurrentAccessContext c,@PathVariable String id){return response(service.detail(c,id));}
    ...
    @PostMapping("/dispatch-orders/{id}/cancellations") public ResponseEntity<DispatchResponse> cancel(@RequestAttribute(ACCESS) CurrentAccessContext c,@PathVariable String id,@RequestHeader(name="If-Match",required=false) String ifMatch,@RequestHeader(name="Idempotency-Key",required=false) String key,@RequestBody(required=false) ReasonRequest r){return mutation(service.cancel(c,id,version(ifMatch),key,r==null?null:r.reason()));}
    ...
    @GetMapping("/my-deliveries/{id}") public DispatchResponse myDelivery(@RequestAttribute(ACCESS) CurrentAccessContext c,@PathVariable String id){return response(service.detail(c,id));}
    ...
    private PageResponse<PodResponse> podPage(LogisticsOperationsService.Page<LogisticsOperationsService.ProofOfDeliveryView> p){return new PageResponse<>(p.items().stream().map(x->new PodResponse(x.dispatchOrderId(),x.dispatchNumber(),x.status(),x.receiverName(),x.completedAt(),x.notes(),x.photoEvidenceDeclared(),x.signatureEvidenceDeclared(),x.updatedAt())).toList(),p.page(),p.size(),p.total());}
    private DispatchResponse response(LogisticsOperationsService.DispatchView x){LogisticsOperationsService.AssignmentView a=x.assignment();return new DispatchResponse(x.id(),x.dispatchNumber(),x.reservationId(),x.salesOrderId(),x.salesOrderNumber(),x.clientAccountId(),x.status(),x.destination(),x.deliveryWindowStart(),x.deliveryWindowEnd(),x.eta(),a==null?null:new AssignmentResponse(a.responsibleMembershipId(),a.responsibleDisplayName(),a.vehicleReference(),a.routeName()),x.temperatureMin(),x.temperatureMax(),x.temperatureUnit(),x.temperatureStatus(),x.podStatus(),x.version(),x.updatedAt(),x.alerts());}
    ...
    private static long version(String value){if(value==null||value.isBlank())throw new LogisticsException("PRECONDITION_REQUIRED",false);try{return Long.parseLong(value.replace("\"",""));}catch(NumberFormatException e){throw new LogisticsException("PRECONDITION_REQUIRED",false);}}

    public record PageResponse<T>(List<T> items,int page,int size,long total){}
    public record DispatchResponse(String id,String dispatchNumber,String reservationId,String salesOrderId,String salesOrderNumber,String clientAccountId,String status,String destination,Instant deliveryWindowStart,Instant deliveryWindowEnd,Instant eta,AssignmentResponse assignment,BigDecimal temperatureMin,BigDecimal temperatureMax,String temperatureUnit,String temperatureStatus,String podStatus,long version,Instant updatedAt,List<String> alerts){}
    ...
    public record PodResponse(String dispatchOrderId,String dispatchNumber,String status,String receiverName,Instant completedAt,String notes,boolean photoEvidenceDeclared,boolean signatureEvidenceDeclared,Instant updatedAt){}
