...
CREATE TABLE sales.purchase_request_material_change (
    ...
    CONSTRAINT fk_pr_material_change_resolver
        ...
        REFERENCES tenant_management.workspace_membership (workspace_id, id),
    CONSTRAINT ck_pr_material_change_status CHECK (status IN ('PROPOSED','ACCEPTED','REJECTED','EXPIRED')),
    CONSTRAINT ck_pr_material_change_resolution CHECK (
        ...
        OR (status IN ('ACCEPTED','REJECTED') AND resolved_at IS NOT NULL AND resolved_by_membership_id IS NOT NULL AND resolved_request_version IS NOT NULL)
        OR (status = 'EXPIRED' AND resolved_at IS NOT NULL AND resolved_request_version IS NOT NULL)
    )
);
...
CREATE FUNCTION sales.expire_open_material_change_on_terminal_request() RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    IF NEW.status IN ('CONVERTED','CONVERTED_TO_ORDER','REJECTED','WITHDRAWN','EXPIRED','CANCELLED')
       AND OLD.status IS DISTINCT FROM NEW.status THEN
        UPDATE sales.purchase_request_material_change
           SET status='EXPIRED', resolved_at=NEW.updated_at, resolved_request_version=NEW.version
         WHERE tenant_id=NEW.tenant_id AND workspace_id=NEW.workspace_id
           AND purchase_request_id=NEW.id AND status='PROPOSED';
    END IF;
    RETURN NEW;
END;
$$;

CREATE TRIGGER trg_purchase_request_expire_open_material_change
AFTER UPDATE OF status ON sales.purchase_request
FOR EACH ROW
EXECUTE FUNCTION sales.expire_open_material_change_on_terminal_request();
